...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchWnbaOddsData = async () => {
      ...
      const proxyUrl = "https://api.allorigins.win/get?url=";
      const targetUrl = encodeURIComponent(
        `https://api.the-odds-api.com/v4/sports/mma_mixed_martial_arts/odds/?apiKey=${apiKey}&regions=us&markets=h2h&oddsFormat=american&bookmakers=unibet
`
      );
      const url = `${proxyUrl}${targetUrl}`;
      const response = await fetch(url);
    ...
    const fetchMlbOddsData = async () => {
      ...
      const proxyUrl = "https://api.allorigins.win/get?url=";
      const targetUrl = encodeURIComponent(
        `https://api.the-odds-api.com/v4/sports/americanfootball_ncaaf/odds/?apiKey=${apiKey}&regions=us&markets=h2h&oddsFormat=american&bookmakers=unibet
`
      );
      const url = `${proxyUrl}${targetUrl}`;
      const response = await fetch(url);
